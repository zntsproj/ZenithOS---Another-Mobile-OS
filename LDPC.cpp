...
// ... (Твой существующий код для LDPC) ...

// --- Функции для работы с 5G модемом ---

// Инициализация подключения
int fiveg_init_connection(const char* device, int baudrate) {
    ...
    tty.c_cflag &= ~PARENB; // Без бита четности
    tty.c_cflag &= ~CSTOPB; // 1 стоп-бит
    tty.c_cflag &= ~CSIZE;
    tty.c_cflag |= CS8;     // 8 бит данных
    tty.c_cflag &= ~CRTSCTS; // Отключение аппаратного управления потоком
    tty.c_cflag |= CREAD | CLOCAL; // Включение приема и игнорирование управляющих линий

    tty.c_lflag &= ~ICANON; // Отключение канонического режима
    tty.c_lflag &= ~ECHO;   // Отключение эха
    tty.c_lflag &= ~ECHOE;  // Отключение стирания
    tty.c_lflag &= ~ISIG;   // Отключение сигналов

    tty.c_iflag &= ~(IXON | IXOFF | IXANY); // Отключение программного управления потоком
    tty.c_iflag &= ~(IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR | IGNCR | ICRNL); // Отключение специальной обработки символов

    tty.c_oflag &= ~OPOST; // Отключение обработки вывода

    tty.c_cc[VMIN]  = 0; // Минимальное количество символов для чтения
    tty.c_cc[VTIME] = 5; // Время ожидания в десятых долях секунды

    cfsetispeed(&tty, baudrate);
    cfsetospeed(&tty, baudrate);

    if (tcsetattr(fd, TCSANOW, &tty) != 0) {
        perror("Ошибка настройки порта");
        return -1;
    }
...
// Отправка AT-команды
int send_at_command(int fd, const char* command, char* response, int response_size) {
    write(fd, command, strlen(command));

    usleep(100000); // Задержка для ответа модема

    int bytes_read = read(fd, response, response_size);
    if (bytes_read == -1) {
    ...
    if (strstr(response, "OK") == NULL) {
        cerr << "Ошибка выполнения команды: " << command << endl;
        return -1;
    }
...
// Подключение к сети
int fiveg_connect(int fd) {
    char command[] = "AT+CGDCONT=1,\"IP\",\"internet\"\r";
    char response[256];
    return send_at_command(fd, command, response, sizeof(response));
}

// Отключение от сети
int fiveg_disconnect(int fd) {
...
int fiveg_get_signal_strength(int fd, int* signal_strength) {
    char command[] = "AT+CSQ\r";
    char response[256];
    int result = send_at_command(fd, command, response, sizeof(response));
    if (result != 0) {
        return result;
    }
...
// Получение имени оператора сети
int fiveg_get_network_operator(int fd, char* operator_name, int buffer_size) {
...
// Получение IP-адреса
int fiveg_get_ip_address(int fd, char* ip_address, int buffer_size) {
...
// Получение IMEI
int fiveg_get_imei(int fd, char* imei, int buffer_size) {
...
// Получение IMSI
int fiveg_get_imsi(int fd, char* imsi, int buffer_size) {
...
int fiveg_check_modem_status(int fd) {
    char command[] = "AT+CSQ\r";
    char response[256];
    int result = send_at_command(fd, command, response, sizeof(response));
    if (result != 0) {
...
int fiveg_get_network_info(int fd) {
    char command[] = "AT+COPS?\r";
    char response[256];
    return send_at_command(fd, command, response, sizeof(response));
}
...
// Настройка APN
int fiveg_set_apn(int fd, const char* apn, const char* username, const char* password) {
    char command[256];
...
// Запуск подключения
int fiveg_start_connection(int fd) {
...
int fiveg_get_ip_address_cifsr(int fd) {
...
int fiveg_deactivate_network(int fd) {
    char command[] = "AT+CGATT=0\r";
    char response[256];
    return send_at_command(fd, command, response, sizeof(response));
}
...
int fiveg_check_connection_status(int fd) {
...
// Перезагрузка модема
int fiveg_restart_modem(int fd) {
    char command[] = "AT+ZRESTART\r";
    char response[256];
    return send_at_command(fd, command, response, sizeof(response));
}

// ... (Другие функции, которые тебе нужны) ...

int main() {
    // ... (Твой код) ...

    // Пример использования функций для 5G модема
    int fd = fiveg_init_connection("/dev/ttyUSB0", B115200);
    if (fd == -1) {
        return -1;
    }
        // ...
